...
export function pickTheyTextedMore(stats: TextStatStats): ContactEntry | null {
  return (
    stats.topContacts
      .filter((c) => c.total >= MIN_TOTAL && c.recv > c.sent)
      .sort((a, b) => b.recv / (b.sent || 1) - a.recv / (a.sent || 1))[0] ?? null
  );
}
...
export function TheyTextedMoreSlide({ stats, palette, isCurrent }: Props) {
  const contact = pickTheyTextedMore(stats);
  if (!contact) return null;

  const ratio = contact.recv / (contact.sent || 1);
  ...
  return (
    <SlideShell label="your biggest fan" palette={palette} isCurrent={isCurrent}>
      ...
      <motion.div {...fu(0.08)} className="text-[0.85rem] uppercase tracking-[0.15em] opacity-65">
        they out-texted you by {ratioStr}
      </motion.div>
      <motion.div
      ...
      >
        <div className="flex h-[72px] rounded-[18px] overflow-hidden border-2 border-black/15">
          <div
            ...
            style={{ flex: contact.recv, backgroundColor: palette.fg, color: contrastColor(palette.fg) }}
          >
          ...
          <div
            className="flex flex-col items-center justify-center min-w-0 px-3 py-1 font-mono text-sm"
            style={{ flex: contact.sent || 1, backgroundColor: "rgba(0,0,0,.25)", color: "rgba(255,255,255,0.9)" }}
          >
